...
import org.ercsn.marketplace.catalog.domain.*;
...
public class MongoEventMetadataRepository implements EventMetadataRepository {
    ...
    private static EventMetadata mapper(org.ercsn.marketplace.catalog.infrastructure.persistence.entity.EventMetadata eventMetadata){
        var sector = eventMetadata.getSectors().stream()
                .map(MongoEventMetadataRepository::mapper)
                .collect(Collectors.toMap(
                        sector1 -> sector1.getId().name(),
                        Function.identity()
                ));
        ...
        var seats = eventMetadata.getSeats().stream()
                ...
                .collect(Collectors.groupingBy(
                        seat -> sector.get(seat.getSectorId().name())
                ));
    ...
    private static Seat mapper(org.ercsn.marketplace.catalog.infrastructure.persistence.entity.EventMetadata.Seat seat) {
        return new Seat(new SeatId(seat.getCode()), new SectorId(seat.getSectorName()));
    }

    private static Sector mapper(org.ercsn.marketplace.catalog.infrastructure.persistence.entity.EventMetadata.Sector sector) {
